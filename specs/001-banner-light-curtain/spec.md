...
2. **Given** 一份包含缺漏或衝突訊號映射的 Light Curtain 組態，**When** 工程師設定或更新 Config，**Then** 系統必須立即拒絕該組態、拋出包含失敗原因的 `ArgumentException`，且不得覆蓋既有有效組態。
...
- **FR-005**：系統必須支援兩種電壓模式，用以表示實際安裝的光幕極性選項，並可回報目前所選模式。此屬性為組態記錄用途，不影響 OSSD 邏輯判讀。
- **FR-006**：系統必須允許呼叫者透過明確方法呼叫按需讀取兩個安全輸入通道與所有支援輸出訊號的目前邏輯狀態，並提供一個 dedicated full-status snapshot method（GetLightCurtainStatus），在單次回應中回傳兩個安全輸入、四個 DO 訊號、運作模式與電壓模式。該方法回傳模組最近一次更新的快取值，不額外觸發硬體 I/O。模組本身不得包含內部輪詢執行緒；輪詢責任由外部呼叫者負責。
...
- **FR-009**：當任何被回報的邏輯訊號、運作模式或電壓模式發生變更時，系統必須發出狀態變更通知；包含 `GetLightCurtainDOStatus` 讀取硬體值與本地快取不同時所觸發的變更。
- **FR-010**：系統必須僅在組態與運作條件允許的情況下，允許呼叫者設定支援的輸出訊號（寫入操作須通過組態、模式與安全狀態三重防護）；讀取操作僅需組態已完成即可執行，不受運作模式與安全狀態限制，以保留診斷可用性。
...
- **SC-004**：在重複狀態查詢驗證中，GetLightCurtainStatus 必須對所有已定義測試案例回傳完整狀態欄位，成功率必須為 100%。
