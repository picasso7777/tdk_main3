...
實作 Banner Light Curtain 模組於 TDKController 專案內，提供兩組安全輸入通道 (OSSD1/OSSD2) 的讀取、四組可控數位輸出 (Reset/Test/Interlock/LTCLed) 的設定與讀取、三種運行模式 (Disable/Enable_InTransfer/Enable_Always)、兩種電壓模式 (24V/0V)、以及安全告警與狀態變更事件通知。模組透過 `IIOBoard[]` 陣列與 `ILogUtility` 進行建構函式注入，使用 `DioChannelConfig` 結構映射 DIO 通道。

## Technical Context

**Language/Version**: C# 7.3 (.NET Framework 4.7.2)
**Primary Dependencies**: DIO 專案 (`IIOBoard`, `IDI`, `IDO` 介面)、TDKLogUtility (`ILogUtility`)
...
| 4 | 建構函式注入 + null-check | ✅ PASS | `ILogUtility` 與 `IIOBoard[]` 注入並驗證 |
...
| 8 | 錯誤碼範圍 -400..-499 | ✅ PASS | 已於 `ErrorCode` 列舉中預留 `LightCurtainError = -400` |
| 9 | 單一 .cs 檔案 | ✅ PASS | 模組實作於 `TDKController/Module/LightCurtain.cs` |
| 10 | 禁止未授權類別 | ✅ PASS | 使用既有 `DioChannelConfig` 結構與 `ErrorCode` 列舉；EventArgs 類別依規格定義 |
...
| 14 | 測試覆蓋率 ≥ 90% | ✅ PLAN | 測試檔案位於 `AutoTest/TDKController.Tests/Unit/LightCurtainTests.cs` |
| 15 | 介面使用政策 | ✅ PASS | 不修改 IConnector、ILogUtility、HRESULT；僅擴充 ILightCurtain |
| 16 | 檔案建立政策 | ⚠️ REVIEW | 需建立：EventArgs 型別（需在已有的 Config/Interface 檔案或 LightCurtain.cs 內定義），以及測試檔 |
...
│   └── ErrorCode.cs             # 錯誤碼列舉（已存在，需新增 LightCurtain 範圍碼）
├── Config/
...
**Structure Decision**: 所有實作程式碼置於已有的 TDKController 專案結構中。不新增專案。DIO 介面唯讀引用。測試檔案置於已有的 `AutoTest/TDKController.Tests/Unit/` 目錄。
